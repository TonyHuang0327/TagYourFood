...
$link = @mysqli_connect( 
    ...
    $GETcateName=$_GET["Categories_Name"];
?>
<!DOCTYPE html>
                ...
                <div class="row justify-content-center">
                    <div class="col-xl-9">
                        <div class="text-center text-white">
                            ...
                            <h2><?php echo $GETcateName;?></h2>
                            <center>
                            <?php
                            $sql = "SELECT F.Food_Id,F.Food_Name,S.Sup_Name,F.Calories,F.Protein,F.Fat,S.Sup_Location,S.Sup_Id 
                                    ...
                                    AND G.Categories_Name='$GETcateName' AND F.Food_Id=C.Food_Id AND C.Sup_Id=S.Sup_Id;";
                            ...
                            while($row=mysqli_fetch_assoc($result)){
                                echo "<tr>";
                                echo "<td><center>".$row["Food_Id"]."</center></td><td><center><a href = 'foodData.php?Food_Name=".$row["Food_Name"]."&Sup_Name=".$row["Sup_Name"]." &Sup_Location=".$row["Sup_Location"]."&Calories=".$row["Calories"]."&Protein=".$row["Protein"]."&Fat=".$row["Fat"]."'>".$row["Food_Name"]."</a></center></td><td><center>".$row["Sup_Name"]."</center></td><td><center>".$row["Calories"]."</center></td><td><center>".$row["Protein"]."</center></td><td><center>".$row["Fat"]."</center></td><td><center><a href = 'addToFavorite.php?Food_Id=".$row["Food_Id"]."'>加到最愛</a></center></td><td><center><a href = 'loginselectConfirm.php?Food_Id=".$row["Food_Id"]."&Sup_Id=".$row["Sup_Id"]."'>選擇</a></center></td>";
                                echo "</tr>";
                            }
                            echo "</table>";
                            ?>
